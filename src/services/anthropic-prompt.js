...
function buildAnthropicToolPrompt(policy, tools) {
  ...
  let prompt = [
    ...
    "=== BEST PRACTICES ===",
    "",
    "1. If a Read result shows ⚠️ FILE TRUNCATED — the file was too long (limit: 12000 chars).",
    "   Immediately switch to Grep for targeted search, or Read with offset/limit in 200-line chunks.",
    "2. After reading any file, check if it starts with ⚠️ — if so, do NOT assume you have full content.",
    "3. Use Grep/Search first to locate code patterns, then Read only the relevant sections.",
    "4. Batch independent file reads in one function_calls for parallel execution.",
    "",
    "=== TOOLS ===",
    "",
    schemas.join("\n\n"),
