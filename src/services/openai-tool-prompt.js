...
function buildToolPrompt(policy, tools) {
  ...
  let prompt = [
    ...
    "=== BEST PRACTICES ===",
    "",
    "1. Large files: use Grep to search for patterns instead of reading entire files.",
    "2. If you must read a large file, use offset/limit params to read it in segments.",
    "3. Read multiple independent files in parallel — batch them in one function_calls.",
    "4. Prefer tools like Grep/Search over Read for finding specific code patterns.",
    "",
    "=== TOOLS ===",
    "",
    toolSchemas.join("\n\n"),
